package net.gobbz.tictactoe.gui;
...
/**
 * Stellt das Spielfeld des Spiels TicTacToe in einem JFrame-Fenster dar. Das
 * Spielfeld
 * wird durch Kn�pfe des Typs TicTacToeJButton dargestellt. Weiters verf�gt das
 * Fenster
 * �ber eine �berschrift, welche denselben Text wie der Fenstertitel enth�lt und
 * eine
...
public class TicTacToeJFrame extends JFrame {
	...
	/**
	 * Konstruktor der die Benutzerschnittstelle in einem Fenster erstellt und
	 * anzeigt.
	 * Dem Konstruktor wird der Titel des Fensters �bergeben. Weiters erh�lt der
	 * Konstruktor
	 * das TicTacToe-Spielobjekt aus welchem dann die Benutzerschnittstelle die
	 * Gr��e
	 * des Spielfeldes und die gesetzten Z�ge ermitteln kann. Dieses Objekt stellt
	 * das
	 * Hauptprogramm dar
	 * 
	 * @param titel     des Fensters
	 * @param tictactoe jenes Objekt welches die Funktionalit�t des Spiels in sich
	 *                  birgt,
	 *                  entweder ein Objekt vom Typ TicTacToeServer oder
	 *                  TicTacToeClient
	 */
	public TicTacToeJFrame(String titel, TicTacToe tictactoe) {
		...
		// Anlegen der Kn�pfe
		JPanel p = new JPanel();
		p.setLayout(new GridLayout(this.tictactoe.getFeldgroesse(), this.tictactoe.getFeldgroesse()));
		b = new TicTacToeJButton[this.tictactoe.getFeldgroesse()][this.tictactoe.getFeldgroesse()];
		int feldnummer = 0;
	...
	/**
	 * Diese Methode liefert nur den ausgew�hlten Zug zur�ck. Sie setzt kein "X"
	 * oder "0"
	 * in den Knopf des Spielfeldes. Diese Methode wartet solange, solange der Zug
	 * nicht
	...
	/**
	 * Setzt ein 'X' oder '0' im zu zug passenden Knopf, wenn noch kein Zeichen ist
	 * @param zug
	 * @param spieler 'X' oder '0'
	 */
	public void setXor0(int zug, char spieler) {
		if (spieler == 'X')
			this.b[zug / this.tictactoe.getFeldgroesse()][zug % this.tictactoe.getFeldgroesse()].setX();
		else if(spieler == '0')
			this.b[zug / this.tictactoe.getFeldgroesse()][zug % this.tictactoe.getFeldgroesse()].set0();
	}
}
